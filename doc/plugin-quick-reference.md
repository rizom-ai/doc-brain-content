---
title: Plugin Quick Reference
section: Customization
order: 130
sourcePath: docs/plugin-quick-reference.md
description: >-
  This document has been consolidated into the repository AGENTS.md files,
  especially entities/AGENTS.md and plugins/AGENTS.md.
slug: plugin-quick-reference
---
# Plugin Development Quick Reference

This document has been consolidated into the repository `AGENTS.md` files, especially [`entities/AGENTS.md`](https://github.com/rizom-ai/brains/blob/main/entities/AGENTS.md) and [`plugins/AGENTS.md`](https://github.com/rizom-ai/brains/blob/main/plugins/AGENTS.md).

See also:

- **Entity plugins**: [`entities/AGENTS.md`](https://github.com/rizom-ai/brains/blob/main/entities/AGENTS.md)
- **Interface plugins**: [`interfaces/AGENTS.md`](https://github.com/rizom-ai/brains/blob/main/interfaces/AGENTS.md)
- **Architecture overview**: [`docs/architecture-overview.md`](/docs/architecture-overview)
