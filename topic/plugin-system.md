...
Typical plugin families include entity plugins for durable content types, service plugins for tools and integrations, and interface plugins for transports and routes such as MCP, Discord, A2A, and webserver endpoints. In application architectures, plugins are the concrete mechanism for extensibility, providing a practical path for future functionality while keeping the core stable.
